...
from djangosige.apps.fiscal.models import ICMS, ICMSSN, IPI, GrupoFiscal, ICMSUFDest
...
class AdicionarGrupoFiscalView(CustomCreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        for key in req_post:
            if "valor" in key or "valiq" in key:
        ...
        if (
            form.is_valid()
            and novo_icms_form.is_valid()
            and icms_dest_form.is_valid()
            and ipi_form.is_valid()
            and pis_form.is_valid()
            and cofins_form.is_valid()
        ):
...
class EditarGrupoFiscalView(CustomUpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if IPI.objects.filter(grupo_fiscal=self.object).count():
            ipi_form = IPIForm(
                request.POST, prefix="ipi_form", grupo_fiscal=self.object
            )
        else:
            ipi_form = IPIForm(request.POST, prefix="ipi_form")
        ...
        if ICMSUFDest.objects.filter(grupo_fiscal=self.object).count():
            pis_form = PISForm(
                request.POST, prefix="pis_form", grupo_fiscal=self.object
            )
        else:
            pis_form = PISForm(request.POST, prefix="pis_form")
