...
try:
    ...
    MD_OPERACAO_NAO_REALIZADA = "210240"
    MD_CIENCIA_OPERACAO = "210210"

TP_MANIFESTO_OPCOES = (
    (MD_CONFIRMACAO_OPERACAO, "Confirmação da Operação"),
    (MD_DESCONHECIMENTO_OPERACAO, "Desconhecimento da Operação"),
    (MD_OPERACAO_NAO_REALIZADA, "Operação Não Realizada"),
    (MD_CIENCIA_OPERACAO, "Ciência da Emissão (ou Ciência da Operação)"),
)
...
class NotaFiscalForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "dhemi": forms.DateTimeInput(
                attrs={"class": "form-control datetimepicker"}, format="%d/%m/%Y %H:%M"
            ),
            "dhsaient": forms.DateTimeInput(
                attrs={"class": "form-control datetimepicker"}, format="%d/%m/%Y %H:%M"
            ),
...
class ConsultarCadastroForm(forms.Form):
    ...
    salvar_arquivos = forms.BooleanField(
        widget=forms.CheckboxInput(
            attrs={
                "class": "form-control",
            }
        ),
        label="Salvar arquivos XML gerados?",
        required=False,
    )
...
class InutilizarNotasForm(forms.Form):
    ...
    empresa = forms.ModelChoiceField(
        ...
        label="Selecionar empresa emitente",
        required=True,
    )
...
class ConsultarNotaForm(forms.Form):
    ...
    nota = forms.ModelChoiceField(
        queryset=NotaFiscalSaida.objects.all(),
        widget=forms.Select(
            attrs={
                "class": "form-control",
            }
        ),
        label="Selecionar nota da base de dados",
        required=False,
    )
...
class BaixarNotaForm(forms.Form):
    ...
    ambiente_nacional = forms.BooleanField(
        widget=forms.CheckboxInput(
            attrs={
                "class": "form-control",
            }
        ),
        label="Utilizar ambiente nacional?(Recomendado)",
        initial=True,
        required=False,
    )
    salvar_arquivos = forms.BooleanField(
...
class ManifestacaoDestinatarioForm(forms.Form):
    cnpj = forms.CharField(
        ...
        label="CNPJ do autor do Evento(apenas digitos)",
        required=True,
    )
    tipo_manifesto = forms.ChoiceField(
        choices=TP_MANIFESTO_OPCOES,
        widget=forms.Select(
            attrs={
                "class": "form-control",
            }
