            ...
            if associated_user:
                try:
                    if associated_user.email:
                        ...
                        subject = "Redefinir sua senha - DjangoSIGE"
                        ...
                        if sended == 1:
                            messages.success(
                                ...
                                + data
                                + ". Aguarde o recebimento da mensagem para trocar sua senha.",
                            )
                            return self.form_valid(form)
                        else:
                            form.add_error(
                                field=None, error="Erro ao enviar email de verificação."
                            )
                            return self.form_invalid(form)
                    else:
                        form.add_error(
                            field=None, error="Este usuário não cadastrou um email."
                        )
                        return self.form_invalid(form)
                ...
                form.add_error(
                    field=None,
                    error="Usuário/Email: {} não foi encontrado na database.".format(
                        data
                    ),
                )
                return self.form_invalid(form)
...
class PasswordResetConfirmView(FormView):
    ...
    def post(self, request, uidb64=None, token=None, *args, **kwargs):
        ...
        if uidb64 is None or token is None:
            form.add_error(
                field=None,
                error="O link usado para a troca de senha não é válido ou expirou, por favor tente enviar novamente.",
            )
            return self.form_invalid(form)
        ...
        if user is not None and default_token_generator.check_token(user, token):
            if form.is_valid():
                ...
                if new_password == new_password_confirm:
                    ...
                    form.add_error(field=None, error="Senhas diferentes.")
                    return self.form_invalid(form)
            else:
                form.add_error(
                    field=None,
                    error="Não foi possivel trocar a senha. Formulário inválido.",
                )
                return self.form_invalid(form)
        else:
            form.add_error(
...
class EditarPerfilView(UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                form=form, minha_empresa_form=minha_empresa_form, object=self.object
            )
        )
    ...
    def post(self, request):
        ...
        return render(
            request,
            self.template_name,
            {"form": form, "minha_empresa_form": minha_empresa_form},
        )
...
class EditarPermissoesUsuarioView(SuperUserRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        condition = reduce(
            operator.or_,
            [Q(codename__icontains=s) for s in ["add_", "change_", "view_", "delete_"]],
        )
        context["default_permissions"] = Permission.objects.filter(
            condition, content_type__model__in=settings.DEFAULT_PERMISSION_MODELS
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse_lazy("login:usuariodetailview", kwargs={"pk": self.kwargs["pk"]})
        )
