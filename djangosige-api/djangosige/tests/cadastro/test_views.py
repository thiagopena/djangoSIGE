...
class CadastroAdicionarViewsTestCase(BaseTestCase):
    def test_add_views_get_request(self):
        for model in CADASTRO_MODELS:
            ...
            url = reverse(f"cadastro:add{model_name}view")
            response = self.client.get(url)
            self.assertEqual(response.status_code, 200)

            # Testar permissao
            permission_codename = "add_" + str(model_name)
            self.check_user_get_permission(url, permission_codename=permission_codename)
    ...
    def test_add_pessoa_post_request(self):
        for model in PESSOA_MODELS:
            ...
            url = reverse(f"cadastro:add{model_name}view")
            pessoa_data = {
                f"{model_name}_form-nome_razao_social": "Razao Social Qualquer",
                f"{model_name}_form-tipo_pessoa": "PJ",
                f"{model_name}_form-inscricao_municipal": "",
                f"{model_name}_form-informacoes_adicionais": "",
            }
            ...
            pessoa_data[f"{model_name}_form-nome_razao_social"] = ""
            response = self.client.post(url, pessoa_data, follow=True)
    ...
    def test_add_produto_post_request(self):
        ...
        produto_data["codigo"] = ""
        response = self.client.post(url, produto_data, follow=True)
        self.assertFormError(response, "form", "codigo", "Este campo é obrigatório.")
    ...
    def test_add_marca_post_request(self):
        ...
        response = self.client.post(url, data, follow=True)
        self.assertFormError(
            response, "form", "marca_desc", "Este campo é obrigatório."
        )
    ...
    def test_list_views_deletar_objeto(self):
        for model in CADASTRO_MODELS:
            ...
            if model_name == "fornecedor":
                ...
                url = reverse(f"cadastro:lista{model_name}sview")
...
class CadastroEditarViewsTestCase(BaseTestCase):
    def test_edit_pessoa_get_post_request(self):
        for model in PESSOA_MODELS:
            ...
            if model_name == "cliente":
                data[
                    "{}-limite_de_credito".format(response.context["form"].prefix)
                ] = data["limite_de_credito"]
    ...
    def test_edit_produto_get_post_request(self):
        ...
        url = reverse("cadastro:editarprodutoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
    ...
    def test_edit_categoria_get_post_request(self):
        ...
        url = reverse("cadastro:editarcategoriaview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
    ...
    def test_edit_marca_get_post_request(self):
        ...
        url = reverse("cadastro:editarmarcaview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
    ...
    def test_edit_unidade_get_post_request(self):
        ...
        url = reverse("cadastro:editarunidadeview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
...
class CadastroAjaxRequestViewsTestCase(BaseTestCase):
    ...
    def test_info_transportadora_post_request(self):
        ...
        self.check_json_response(url, data, obj_pk, model="cadastro.transportadora")
    ...
    def test_info_fornecedor_post_request(self):
        ...
        data = {"pessoaId": obj_pk}
        self.check_json_response(url, data, obj_pk, model="cadastro.fornecedor")
