...
class FinanceiroFluxoCaixaViewTestCase(BaseTestCase):
    ...
    def test_fluxo_caixa_get_request_data_inicial(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            list(response.context["movimentos"]),
            list(
...
class FinanceiroAdicionarViewsTestCase(BaseTestCase):
    ...
    def test_add_grupo_view_post_request(self):
        ...
        response = self.client.post(url, data, follow=True)
        self.assertFormError(response, "form", "descricao", "Este campo é obrigatório.")
    ...
    def test_add_conta_pagar_view_post_request(self):
        ...
        # adicionada
        movimento_criado = MovimentoCaixa.objects.get(data_movimento=data_atual)
...
class FinanceiroPlanoContasViewsTestCase(BaseTestCase):
    ...
    def test_deletar_subgrupo_plano_contas(self):
        ...
        PlanoContasSubgrupo.objects.create(
            codigo="000010",
            tipo_grupo="0",
            descricao="Subgrupo Entrada Teste",
            grupo=grupo,
        )
...
class FinanceiroListarViewsTestCase(BaseTestCase):
    ...
    def test_list_conta_pagar_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(
            url=reverse("financeiro:listacontapagarview"), deleted_object=obj
        )
    ...
    def test_list_conta_pagar_atrasada_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(
            url=reverse("financeiro:listacontapagaratrasadasview"), deleted_object=obj
        )

    def test_list_conta_pagar_hoje_view_deletar_objeto(self):
        obj = Saida.objects.create(status="1", data_vencimento=datetime.today())
        self.check_list_view_delete(
            url=reverse("financeiro:listacontapagarhojeview"), deleted_object=obj
        )

    def test_list_conta_receber_view_deletar_objeto(self):
        obj = Entrada.objects.create(status="1")
        self.check_list_view_delete(
            url=reverse("financeiro:listacontareceberview"), deleted_object=obj
        )
    ...
    def test_list_conta_receber_atrasada_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(
            url=reverse("financeiro:listacontareceberatrasadasview"), deleted_object=obj
        )

    def test_list_conta_receber_hoje_view_deletar_objeto(self):
        obj = Entrada.objects.create(status="1", data_vencimento=datetime.today())
        self.check_list_view_delete(
            url=reverse("financeiro:listacontareceberhojeview"), deleted_object=obj
        )
    ...
    def test_list_recebimento_view_deletar_objeto(self):
        ...
        try:
            ultimo_mvmt = MovimentoCaixa.objects.filter(
                data_movimento__lt=novo_movimento.data_movimento
            ).latest("data_movimento")
            novo_movimento.saldo_inicial = ultimo_mvmt.saldo_final
            novo_movimento.saldo_final = novo_movimento.saldo_inicial + Decimal(
                "120.00"
            )
        ...
        novo_movimento.save()
        obj = Entrada.objects.create(
            descricao="Nova Entrada Teste",
            status="0",
            valor_total=Decimal("120.00"),
            valor_liquido=Decimal("120.00"),
            movimentar_caixa=True,
            movimento_caixa=novo_movimento,
        )
        self.check_list_view_delete(
            url=reverse("financeiro:listarecebimentosview"), deleted_object=obj
        )

        # Verificar movimento deletado
        self.assertFalse(MovimentoCaixa.objects.filter(pk=novo_movimento.pk).exists())
    ...
    def test_list_pagamento_view_deletar_objeto(self):
        ...
        self.check_list_view_delete(
            url=reverse("financeiro:listapagamentosview"), deleted_object=obj
        )
...
class FinanceiroEditarViewsTestCase(BaseTestCase):
    def test_edit_grupo_get_post_request(self):
        ...
        url = reverse("financeiro:editargrupoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_conta_pagar_get_post_request(self):
        ...
        url = reverse("financeiro:editarcontapagarview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
    ...
    def test_edit_conta_receber_get_post_request(self):
        ...
        url = reverse("financeiro:editarcontareceberview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
        replace_none_values_in_dictionary(data)
    ...
    def test_edit_recebimento_com_data_pagamento_igual_get_post_request(self):
        ...
        obj = (
            Entrada.objects.filter(status="0", movimentar_caixa=True)
            .exclude(Q(movimento_caixa__isnull=True) | Q(data_pagamento__isnull=True))
            .order_by("pk")
            .last()
        )
        valor_saldo_final_antigo = obj.movimento_caixa.saldo_final
        data_movimento = obj.movimento_caixa.data_movimento
        url = reverse("financeiro:editarrecebimentoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
        replace_none_values_in_dictionary(data)
        ...
        data["valor_total"] = locale.format(
            "%.2f", Decimal(data["valor_total"]) + Decimal("20.00"), 1
        )
        data["valor_liquido"] = locale.format(
            "%.2f", Decimal(data["valor_liquido"]) + Decimal("20.00"), 1
        )
        response = self.client.post(url, data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "financeiro/lancamento/lancamento_list.html")
        ...
        movimento_editado = MovimentoCaixa.objects.get(data_movimento=data_movimento)
        self.assertEqual(
            valor_saldo_final_antigo + Decimal("20.00"), movimento_editado.saldo_final
        )

    def test_edit_recebimento_com_data_pagamento_diferente_e_movimento_criado_get_post_request(
        self,
    ):
        # Buscar entrada com movimento de caixa e data_pagamento
        ...
        obj = (
            Entrada.objects.filter(status="0", movimentar_caixa=True)
            .exclude(
                Q(movimento_caixa__isnull=True)
                | Q(data_pagamento__isnull=True)
                | Q(data_pagamento=datetime.strptime("06/07/2017", "%d/%m/%Y").date())
            )
            .order_by("pk")
            .last()
        )
        # Data aleatoria, que nao possui movimentos de caixa
        nova_data_pagamento = datetime.strptime("01/01/2020", "%d/%m/%Y").date()
        url = reverse("financeiro:editarrecebimentoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
    ...
    def test_edit_recebimento_com_data_pagamento_diferente_e_movimento_existe_get_post_request(
        self,
    ):
        # Buscar entrada com movimento de caixa e data_pagamento
        # Modificar a data_pagamento atual, para buscar outro movimento ou
        ...
        nova_data_pagamento = datetime.strptime("06/07/2017", "%d/%m/%Y").date()
        url = reverse("financeiro:editarrecebimentoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
        novo_valor_entradas = (
            MovimentoCaixa.objects.get(data_movimento=nova_data_pagamento).entradas
            + Decimal(data["valor_liquido"])
            + Decimal("20.00")
        )
        replace_none_values_in_dictionary(data)
        data["descricao"] = "Recebimento editado"
    ...
    def test_edit_recebimento_movimentar_caixa_false_get_post_request(self):
        ...
        replace_none_values_in_dictionary(data)
        data["descricao"] = "Recebimento editado"
        data["valor_total"] = locale.format("%.2f", Decimal(data["valor_total"]), 1)
        data["valor_liquido"] = locale.format("%.2f", Decimal(data["valor_liquido"]), 1)
    ...
    def test_edit_recebimento_movimentar_caixa_true_get_post_request(self):
        ...
        data_pagamento_futura = datetime.strptime("01/01/2099", "%d/%m/%Y").date()
        obj = Entrada.objects.create(
            status="0",
    ...
    def test_edit_pagamento_com_data_pagamento_igual_get_post_request(self):
        ...
        # Nao modificar a data_pagamento atual, para buscar o mesmo movimento
        obj = (
            Saida.objects.filter(status="0", movimentar_caixa=True)
            .exclude(Q(movimento_caixa__isnull=True) | Q(data_pagamento__isnull=True))
            .order_by("pk")
            .last()
        )
        valor_saldo_final_antigo = obj.movimento_caixa.saldo_final
        data_movimento = obj.movimento_caixa.data_movimento
        url = reverse("financeiro:editarpagamentoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
        replace_none_values_in_dictionary(data)
    ...
    def test_edit_pagamento_com_data_pagamento_diferente_e_movimento_criado_get_post_request(
        ...
        # Data aleatoria, que nao possui movimentos de caixa
        nova_data_pagamento = datetime.strptime("01/01/2030", "%d/%m/%Y").date()
        url = reverse("financeiro:editarpagamentoview", kwargs={"pk": obj.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        data = response.context["form"].initial
    ...
    def test_edit_pagamento_com_data_pagamento_diferente_e_movimento_existe_get_post_request(
        self,
    ):
        # Buscar saida com movimento de caixa e data_pagamento
        # Modificar a data_pagamento atual, para buscar outro movimento ou
        ...
        data = response.context["form"].initial
        novo_valor_saidas = (
            MovimentoCaixa.objects.get(data_movimento=nova_data_pagamento).saidas
            + Decimal(data["valor_liquido"])
            + Decimal("20.00")
        )
        replace_none_values_in_dictionary(data)
    ...
    def test_edit_pagamento_movimentar_caixa_true_get_post_request(self):
        # Criar lancamento com data futura (sem movimentos)
        data_pagamento_futura = datetime.strptime("01/01/2020", "%d/%m/%Y").date()
        obj = Saida.objects.create(
            status="0",
            movimentar_caixa=False,
            valor_total="120.00",
...
class FinanceiroGerarLancamentoViewTestCase(BaseTestCase):
    ...
    def test_gerar_recebimento_a_partir_de_conta_receber_mesma_data_pagamento(self):
        # Buscar objeto qualquer
        obj = (
            Entrada.objects.filter(status="1", movimentar_caixa=True)
            .exclude(
                Q(movimento_caixa__isnull=True)
                | (Q(data_pagamento__isnull=True) & Q(data_vencimento__isnull=True))
            )
            .order_by("pk")
            .last()
        )
        ...
        self.assertEqual(
            ...
            reverse("financeiro:editarrecebimentoview", kwargs={"pk": obj.id}),
        )
    ...
    def test_gerar_recebimento_a_partir_de_conta_receber_muda_data_pagamento(self):
        ...
        movimento_criado = MovimentoCaixa.objects.get(data_movimento=data_pagamento)
    ...
    def test_gerar_pagamento_a_partir_de_conta_pagar_mesma_data_pagamento(self):
        # Buscar objeto qualquer
        obj = (
            Saida.objects.filter(status="1", movimentar_caixa=True)
            .exclude(
                Q(movimento_caixa__isnull=True)
                | (Q(data_pagamento__isnull=True) & Q(data_vencimento__isnull=True))
            )
        ...
        self.assertEqual(
            response_content["url"],
            reverse("financeiro:editarpagamentoview", kwargs={"pk": obj.id}),
        )
...
class FinanceiroFaturarPedidoViewsTestCase(BaseTestCase):
    def test_faturar_pedido_venda(self):
        ...
        response = self.client.get(
            url, follow=True, HTTP_REFERER=reverse("financeiro:listalancamentoview")
        )
        self.assertEqual(response.status_code, 200)

        # Verificar se estoque dos produtos foi movimentado
        self.assertTrue(
            SaidaEstoque.objects.filter(
                observacoes="Saída de estoque pelo pedido de venda nº4"
            ).exists()
        )
