...
$.Admin.table = {
    init: function() {
        ...
        $btnRemove.on('click',function(event){
            ...
            //MsgRemove("Os items selecionados serão removidos permanentemente da Base de Dados.");
        ...
        //$(".clickable-row:not(.popup)").on('click', function(event) {
...
$.Admin.formset = {
            ...
            currentBox.on('click', '.remove-formset', function(e){
                ...
                if(entryId.length){
                    ...
                    parentFormset.find(':input').each(function(){
                        //$(this).attr({'value': ''}).val('').removeAttr('checked');
    ...
    createNewForm: function(parentFormset, nFormsets){
        ...
        //Trocar id
        var nameRegex = /-\d+-?/g;
        //var newId = newForm.attr('id').replace('-' + (nFormsets-1), '-' + (nFormsets));
        ...
        newForm.find(':input').each(function(){
            ...
            if(newName){
                //newName = newName.replace('-' + (nFormsets-1) + '-', '-' + (nFormsets) + '-');
        ...
        newForm.find('label').each(function() {
            ...
            if(newFor){
                //newFor = newFor.replace('-' + (nFormsets - 1) + '-','-' + nFormsets + '-');
    ...
    createNewTrForms: function(table, n_new_forms){
        ...
        //table.find('tbody tr:first-child').addClass('hidden');
        ...
        //table.find('tbody tr:first-child input').each(function(){
        ...
        if(typeof n_new_forms != 'undefined' || n_new_forms){
            ...
            //for (i = 0; i < parseInt(n_new_forms) -1; i++) {
            for (i = 0; i < parseInt(n_new_forms); i++) {
                ...
                trClones.each(function(){
                    //var newId = $(this).attr('id').replace(nameRegex, '-' + (i+1));
                    ...
                    $(this).find('input').each(function(){
                        ...
                        if(newName){
                            //newName = newName.replace(nameRegex, '-' + (i+1) + '-');
                            newName = newName.replace(nameRegex, '-' + (nameIdNumber) + '-');
                            var newId = 'id_' + newName;
                            //$(this).attr({'name': newName, 'id': newId, 'value': ''}).val('').removeAttr('checked');
...
$.Admin.maskInput = {
    ...
    maskNotaFiscal: function(){
        ...
        //$('.datetimepicker').mask('00/00/0000 00:00', {reverse: true});
    },
...
$.Admin.grupoFiscalForm = {
    ...
    escondeCamposIcmsSn: function (icmssnCsosnField) {
        ...
        //$('.icmssn_form:not(.icmssn-csosn)').hide();
...
$.Admin.vendaForm = {
    init: function(req_urls, invalid) {
        ...
        cli_input.on('change', function(){
            if($(this).val()){
                ...
                //_this.ajaxPostRequest(req_urls['info_cliente_url'], postData, _this.handleClienteInfo);
        ...
        if(form_atual.find('select[id$=-produto]').val() && form_atual.find('input[id$=-auto_calcular_impostos]').is(':checked')){
            ...
            if(parseFloat(picms) > 0){
                if(form_atual.find('input[id$=-icms_incluido_preco]').is(':checked')){
                    ...
                    //var vbc_icms_red = parseFloat(vbc_icms) - parseFloat(vbc_icms)*parseFloat(red_bc_icms);
            ...
            if(parseFloat(picmsst) > 0){
                if(form_atual.find('input[id$=-icmsst_incluido_preco]').is(':checked')){
                    ...
                    //var vbc_icms_st_red = parseFloat(vbc_icms_st) - parseFloat(vbc_icms_st)*parseFloat(red_bc_icmsst);
    ...
    salvarInfoImpostoModal: function(modal){
        ...
        //form_atual.find('input[id$=-p_red_bc]').val(modal.find('#id_p_red_bc').val());
        ...
        //form_atual.find('input[id$=-p_mvast]').val(modal.find('#id_pmvast_modal').val());
        //form_atual.find('input[id$=-p_red_bcst]').val(modal.find('#id_p_red_bcst_modal').val());
        ...
        //form_atual.find('input[id$=-total_impostos]').val(modal.find('#id_total_impostos_modal').val());
        //form_atual.find('input[id$=-subtotal]').val(modal.find('#id_subtotal_modal').val());
        //form_atual.find('input[id$=-total_com_impostos]').val(modal.find('#id_total_modal').val());
            ...
            for(var i = 0; i < data.length; i++) {
                ...
                if(data[i].model == 'fiscal.icms' && !initial){
                    ...
                    if(data[i].fields.tipo_ipi == '0'  && !initial){
                        ...
                        //if(data[i].fields.valor_fixo) form_atual.find('input[id$=-vipi]').val(data[i].fields.valor_fixo);
        ...
        if(data){
            ...
            for(var i = 0; i < data.length; i++) {
                if(data[i].model == 'vendas.itensvenda' || data[i].model == 'compras.itenscompra'){
                    ...
                    //$('#id_quantidade_itens').val(vitens.toString());
                    //$('#id_valor_total').val(data[i].fields.valor_total_produtos);
                }
            }
