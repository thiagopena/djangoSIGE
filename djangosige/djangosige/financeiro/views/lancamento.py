...
class ContaPagarListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        return super(ContaPagarListView, self).get_queryset(
            object=Saida, status=["1", "2"]
        )
...
class ContaPagarHojeListView(ContaPagarAtrasadasListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(ContaPagarHojeListView, self).get_context_data(**kwargs)
        context[
            "title_complete"
...
class ContaReceberListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        return super(ContaReceberListView, self).get_queryset(
            object=Entrada, status=["1", "2"]
        )
...
class EntradaListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        return super(EntradaListView, self).get_queryset(
            object=Entrada,
            status=[
                "0",
            ],
...
class SaidaListView(LancamentoListBaseView):
    ...
    def get_queryset(self):
        return super(SaidaListView, self).get_queryset(
            object=Saida,
            status=[
                "0",
            ],
...
class FaturarPedidoVendaView(CustomView, MovimentoCaixaMixin):
    ...
    def atualizar_estoque(self, request, pedido):
        ...
        if not erros:
            ...
            saida_estoque.observacoes = (
                "Saída de estoque pelo pedido de venda nº{}".format(str(pedido.id))
            )
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.success(
            request,
            "<b>Pedido de venda {0} </b>faturado com sucesso.".format(str(pedido.id)),
        )
...
class FaturarPedidoCompraView(CustomView, MovimentoCaixaMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.success(
            request,
            "<b>Pedido de compra {0} </b>realizado com sucesso.".format(str(pedido.id)),
        )
