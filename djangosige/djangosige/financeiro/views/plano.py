                ...
                if value == "on":
                    ...
                    if grupo and subgrupo:
                        for i, obj in enumerate(
                            PlanoContasSubgrupo.objects.filter(grupo=grupo), start=1
                        ):
                    ...
                    # Reordenar codigos dos grupos e subgrupos
                        ...
                        for i, obj in enumerate(
                            PlanoContasGrupo.objects.filter(pk__in=id_list), start=1
                        ):
                            obj.codigo = str(i)
                            obj.save()
                            for j, subobj in enumerate(
                                PlanoContasSubgrupo.objects.filter(grupo=obj), start=1
                            ):
...
class AdicionarGrupoPlanoContasView(CustomCreateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(form=form, formset=subgrupo_form)
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() and subgrupo_form.is_valid():
            ...
            n_subgrupos = PlanoContasSubgrupo.objects.filter(
                tipo_grupo=self.object.tipo_grupo
            ).count()
...
class EditarGrupoPlanoContasView(CustomUpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() and subgrupo_form.is_valid():
            ...
            for i, obj in enumerate(
                PlanoContasSubgrupo.objects.filter(grupo=self.object), start=1
            ):
                obj.codigo = str(self.object.codigo) + "." + str(i)
                obj.tipo_grupo = self.object.tipo_grupo
                obj.save()
