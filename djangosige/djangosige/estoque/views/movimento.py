...
class MovimentoEstoqueMixin(object):
    def adicionar_novo_movimento_estoque(
        self, itens_mvmt_obj, pform, lista_produtos, lista_produtos_estocados
    ):
        ...
        if prod.estoque_atual is not None and isinstance(self.object, EntradaEstoque):
            ...
            if prod.estoque_atual < itens_mvmt_obj.quantidade:
                pform.add_error(
                    "quantidade",
                    "Quantidade retirada do estoque maior que o estoque atual ("
                    + str(prod.estoque_atual).replace(".", ",")
                    + ") do produto.",
                )
            else:
                prod.estoque_atual = prod.estoque_atual - itens_mvmt_obj.quantidade
            ...
            if itens_mvmt_obj.quantidade > prod_estocado_orig.quantidade:
                ...
                prod_estocado_orig.quantidade = (
                    prod_estocado_orig.quantidade - itens_mvmt_obj.quantidade
                )

            prod_estocado_dest.quantidade = (
                prod_estocado_dest.quantidade + itens_mvmt_obj.quantidade
            )
...
class AdicionarMovimentoEstoqueBaseView(CustomCreateView, MovimentoEstoqueMixin):
    ...
    def get_context_data(self, **kwargs):
        context = super(AdicionarMovimentoEstoqueBaseView, self).get_context_data(
            **kwargs
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        for key in req_post:
            if "quantidade" in key or "valor" in key or "total" in key:
...
class AdicionarEntradaEstoqueView(AdicionarMovimentoEstoqueBaseView):
    ...
    success_message = (
        "<b>Movimento de estoque de entrada nº%(pk)s</b> adicionado com sucesso."
    )
...
class AdicionarSaidaEstoqueView(AdicionarMovimentoEstoqueBaseView):
    ...
    success_message = (
        "<b>Movimento de estoque de saída nº%(pk)s</b> adicionado com sucesso."
    )
...
class AdicionarTransferenciaEstoqueView(AdicionarMovimentoEstoqueBaseView):
    ...
    success_message = (
        "<b>Movimento de estoque de transferência nº%(pk)s</b> adicionado com sucesso."
    )
...
class DetalharMovimentoEstoqueBaseView(CustomDetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super(DetalharMovimentoEstoqueBaseView, self).get_context_data(
            **kwargs
        )
        return self.view_context(context)
...
class DetalharEntradaEstoqueView(DetalharMovimentoEstoqueBaseView):
    ...
    def view_context(self, context):
        context["title_complete"] = "MOVIMENTO DE ENTRADA EM ESTOQUE N°" + str(
            self.object.id
        )
        context["return_url"] = reverse_lazy("estoque:listaentradasestoqueview")
        return context
...
class DetalharSaidaEstoqueView(DetalharMovimentoEstoqueBaseView):
    ...
    def view_context(self, context):
        context["title_complete"] = "MOVIMENTO DE SAÍDA EM ESTOQUE N°" + str(
            self.object.id
        )
        context["return_url"] = reverse_lazy("estoque:listasaidasestoqueview")
        return context
...
class DetalharTransferenciaEstoqueView(DetalharMovimentoEstoqueBaseView):
    ...
    def view_context(self, context):
        context["title_complete"] = "MOVIMENTO DE TRANSFERÊNCIA EM ESTOQUE N°" + str(
            self.object.id
        )
        context["return_url"] = reverse_lazy("estoque:listatransferenciasestoqueview")
        return context
