...
class BaseTestCase(TestCase):
    ...
    def setUp(self):
        try:
            self.user = User.objects.get(username=TEST_USERNAME, email=TEST_EMAIL)
    ...
    def check_user_get_permission(self, url, permission_codename):
        ...
        message_tags = " ".join(str(m.tags) for m in list(response.context["messages"]))
    ...
    def check_list_view_delete(
        self, url, deleted_object, context_object_key="object_list"
    ):
