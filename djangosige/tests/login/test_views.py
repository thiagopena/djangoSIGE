...
from tests.test_case import TEST_PASSWORD, TEST_USERNAME, BaseTestCase
...
class UserRegistrationFormViewTestCase(BaseTestCase):
    ...
    def test_user_registration(self):
        ...
        response = self.client.post(url, data, follow=True)
        self.assertFormError(response, "form", "password", "Senhas diferentes.")
...
class EditarPerfilViewTestCase(BaseTestCase):
    def test_editar_perfil_view(self):
        ...
        self.assertEqual(usuario.empresa_usuario.all()[0].m_empresa.pk, m_empresa.pk)
...
class SelecionarMinhaEmpresaViewTestCase(BaseTestCase):
    def test_selecionar_empresa_view(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "login/selecionar_minha_empresa.html")

        data = {
            "m_empresa": m_empresa.pk,
        }
        response = self.client.post(url, data, follow=True)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(usuario.empresa_usuario.all()[0].m_empresa.pk, m_empresa.pk)
