    ...
    def get(self, request, form_class, *args, **kwargs):
        ...
        return self.render_to_response(
            self.get_context_data(
                form=form, produtos_form=produtos_form, pagamento_form=pagamento_form
            )
        )
...
class OrcamentoCompraVencidosListView(OrcamentoCompraListView):
    ...
    def get_queryset(self):
        return OrcamentoCompra.objects.filter(
            data_vencimento__lte=datetime.now().date(), status="0"
        )
...
class OrcamentoCompraVencimentoHojeListView(OrcamentoCompraListView):
    ...
    def view_context(self, context):
        context[
            "title_complete"
        ] = "ORÇAMENTOS DE COMPRA COM VENCIMENTO DIA " + datetime.now().date().strftime(
            "%d/%m/%Y"
        )
        context["add_url"] = reverse_lazy("compras:addorcamentocompraview")
        return context
    ...
    def get_queryset(self):
        return OrcamentoCompra.objects.filter(
            data_vencimento=datetime.now().date(), status="0"
        )
...
class PedidoCompraAtrasadosListView(PedidoCompraListView):
    ...
    def get_queryset(self):
        return PedidoCompra.objects.filter(
            data_entrega__lte=datetime.now().date(), status="0"
        )
...
class PedidoCompraEntregaHojeListView(PedidoCompraListView):
    ...
    def view_context(self, context):
        context[
            "title_complete"
        ] = "PEDIDOS DE COMPRA COM ENTREGA DIA " + datetime.now().date().strftime(
            "%d/%m/%Y"
        )
        context["add_url"] = reverse_lazy("compras:addpedidocompraview")
        return context
...
class EditarCompraView(CustomUpdateView):
    ...
    def get(self, request, form_class, *args, **kwargs):
        ...
        produtos_form.initial = [
            {
                "total_sem_desconto": item.get_total_sem_desconto(),
                "total_impostos": item.get_total_impostos(),
                "total_com_impostos": item.get_total_com_impostos(),
            }
...
class EditarOrcamentoCompraView(EditarCompraView):
    ...
    def view_context(self, context):
        context["title_complete"] = "EDITAR ORÇAMENTO DE COMPRA N°" + str(
            self.object.id
        )
        context["return_url"] = reverse_lazy("compras:listaorcamentocompraview")
        return context
...
class GerarPedidoCompraView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse_lazy(
                "compras:editarpedidocompraview", kwargs={"pk": novo_pedido.id}
            )
        )
...
class CancelarOrcamentoCompraView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse_lazy(
                "compras:editarorcamentocompraview", kwargs={"pk": instance.id}
            )
        )
...
class CancelarPedidoCompraView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        instance.save()
        return redirect(
            reverse_lazy("compras:editarpedidocompraview", kwargs={"pk": instance.id})
        )
...
class ReceberPedidoCompraView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        messages.success(
            request, f"<b>Pedido de compra {str(pedido.id)} </b>recebido com sucesso."
        )
...
class GerarPDFCompra(CustomView):
    def gerar_pdf(self, title, compra, user_id):
        ...
        try:
            ...
            logo_path = f"{MEDIA_ROOT}{flogo.name}"
