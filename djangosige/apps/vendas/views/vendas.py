...
class OrcamentoVendaVencidosListView(OrcamentoVendaListView):
    ...
    def get_queryset(self):
        return OrcamentoVenda.objects.filter(
...
class OrcamentoVendaVencimentoHojeListView(OrcamentoVendaListView):
    ...
    def view_context(self, context):
        context[
            "title_complete"
        ] = "ORÇAMENTOS DE VENDA COM VENCIMENTO DIA " + datetime.now().date().strftime(
            "%d/%m/%Y"
        )
        context["add_url"] = reverse_lazy("vendas:addorcamentovendaview")
        return context
...
class PedidoVendaAtrasadosListView(PedidoVendaListView):
    ...
    def get_queryset(self):
        return PedidoVenda.objects.filter(
...
class PedidoVendaEntregaHojeListView(PedidoVendaListView):
    ...
    def view_context(self, context):
        context[
            "title_complete"
        ] = "PEDIDOS DE VENDA COM ENTREGA DIA " + datetime.now().date().strftime(
            "%d/%m/%Y"
        )
        context["add_url"] = reverse_lazy("vendas:addpedidovendaview")
        return context
...
class GerarPedidoVendaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse_lazy("vendas:editarpedidovendaview", kwargs={"pk": novo_pedido.id})
        )
...
class CancelarOrcamentoVendaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        instance.save()
        return redirect(
            reverse_lazy("vendas:editarorcamentovendaview", kwargs={"pk": instance.id})
        )
...
class CancelarPedidoVendaView(CustomView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        instance.save()
        return redirect(
            reverse_lazy("vendas:editarpedidovendaview", kwargs={"pk": instance.id})
        )
