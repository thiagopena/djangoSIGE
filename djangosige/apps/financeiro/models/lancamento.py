...
class Lancamento(models.Model):
    ...
    conta_corrente = models.ForeignKey(
        "cadastro.Banco",
        related_name="conta_corrente_conta",
    ...
    abatimento = models.DecimalField(
        max_digits=13,
        decimal_places=2,
        validators=[MinValueValidator(Decimal("0.00"))],
        default=Decimal("0.00"),
    ...
    movimento_caixa = models.ForeignKey(
        "financeiro.MovimentoCaixa",
        related_name="movimento_caixa_lancamento",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    ...
    def format_valor_liquido(self):
        return locale.format("%.2f", self.valor_liquido, 1)
...
class Entrada(Lancamento):
    ...
    grupo_plano = models.ForeignKey(
        ...
        related_name="grupo_plano_recebimento",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    ...
    def get_edit_url(self):
        if self.status == "0":
            return reverse_lazy(
                "financeiro:editarrecebimentoview", kwargs={"pk": self.id}
            )
        else:
            return reverse_lazy(
                "financeiro:editarcontareceberview", kwargs={"pk": self.id}
            )
...
class Saida(Lancamento):
    ...
    grupo_plano = models.ForeignKey(
        "financeiro.PlanoContasGrupo",
        related_name="grupo_plano_pagamento",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    ...
    def get_edit_url(self):
        if self.status == "0":
            return reverse_lazy(
                "financeiro:editarpagamentoview", kwargs={"pk": self.id}
            )
        else:
            return reverse_lazy(
                "financeiro:editarcontapagarview", kwargs={"pk": self.id}
            )
...
class MovimentoCaixa(models.Model):
    ...
    def __unicode__(self):
        s = "Movimento dia %s" % (self.data_movimento)
        return s
