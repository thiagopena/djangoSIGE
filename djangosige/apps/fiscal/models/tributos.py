...
class ICMS(models.Model):
    ##Nota Fiscal
    cst             = models.CharField(max_length=3, choices=CST_ICMS_ESCOLHAS, help_text='icms-cst')
...
class IPI(models.Model):
    cst          = models.CharField(max_length=2, choices=CST_IPI_ESCOLHAS, null=True, blank=True)
...
class PIS(models.Model):
    ...
    valiq_pis    = models.DecimalField(max_digits=13, decimal_places=2, validators=[MinValueValidator(Decimal('0.00'))], null=True, blank=True)
    grupo_fiscal = models.ForeignKey('fiscal.GrupoFiscal', related_name="pis_padrao", on_delete=models.CASCADE, null=True, blank=True)
