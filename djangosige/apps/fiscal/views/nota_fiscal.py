...
class NotaFiscalViewMixin(object):
    def atualizar_campos(self, post_data):
        values_dict = {}
        itens_id = []
        decimal_fields = ['vq_bcpis', 'vq_bccofins', 'vpis', 'vcofins', 'vicms_deson',]
...
class EditarNotaFiscalSaidaView(EditarNotaFiscalView):
    ...
    def view_context(self, context):
        context['title_complete'] = 'EDITAR NOTA FISCAL DE SAÍDA ' + str(self.object.serie) + '/' + str(self.object.n_nf_saida)
        context['return_url'] = reverse_lazy('fiscal:listanotafiscalsaidaview')
        context['saida'] = True
        return context
...
class EditarNotaFiscalEntradaView(EditarNotaFiscalView):
    ...
    def view_context(self, context):
        context['title_complete'] = 'EDITAR NOTA FISCAL DE ENTRADA ' + str(self.object.serie) + '/' + str(self.object.n_nf_entrada)
...
class GerarNotaFiscalSaidaView(View):
    def get(self, request, *args, **kwargs):
        ...
        try:
            nova_nota.emit_saida = MinhaEmpresa.objects.get(m_usuario=Usuario.objects.get(user=request.user)).m_empresa
        except:
            pass
...
class ConfiguracaoNotaFiscalView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        self.object = self.get_object()
        form = ConfiguracaoNotaFiscalForm(request.POST, request.FILES, instance=self.object)
...
class EmitirNotaView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['saida'] = True
        context['return_url'] = reverse_lazy('fiscal:editarnotafiscalsaidaview', kwargs={'pk':self.object.pk})
        return context
    ...
    def post(self, request, *args, **kwargs):
        if len(request.FILES):
            ...
            try:
                ...
                messages.error(request, 'O seguinte erro foi encontrado ao tentar ler o arquivo XML: ' + str(e))
        else:
...
class InutilizarNotasView(TemplateView):
    ...
    def inutilizar_notas(self, empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos):
        processador_nota = ProcessadorNotaFiscal()
        processador_nota.inutilizar_notas(empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            processo = self.inutilizar_notas(empresa, ambiente, modelo, serie, numero_inicial, numero_final, justificativa, salvar_arquivos)
            return self.form_valid(form, processo)
...
class ConsultarNotaView(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if chave and nota:
                messages.error(self.request, 'Preencha apenas um dos campos (Consultar nota da base de dados ou por chave).')
                return self.form_invalid(form)
            elif not chave and not nota:
                messages.error(self.request, 'Preencha ao menos um dos campos: \'Selecionar nota da base de dados\' ou \'Chave da nota\'.')
                return self.form_invalid(form)
            elif nota:
                chave = nota.chave
...
class BaixarNotaView(TemplateView):
    ...
    def baixar_nota(self, chave, ambiente, ambiente_nacional, salvar_arquivos):
        processador_nota = ProcessadorNotaFiscal()
        processador_nota.baixar_nota(chave, ambiente, ambiente_nacional, salvar_arquivos)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if chave and nota:
                messages.error(self.request, 'Preencha apenas um dos campos (Baixar nota da base de dados ou por chave).')
                return self.form_invalid(form)
            elif not chave and not nota:
                messages.error(self.request, 'Preencha ao menos um dos campos: \'Selecionar nota da base de dados\' ou \'Chave da nota\'.')
                return self.form_invalid(form)
            ...
            processo = self.baixar_nota(chave, ambiente, ambiente_nacional, salvar_arquivos)
            return self.form_valid(form, processo)
...
class ManifestacaoDestinatarioView(TemplateView):
    ...
    def efetuar_manifesto(self, chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos):
        processador_nota = ProcessadorNotaFiscal()
        processador_nota.efetuar_manifesto(chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if tipo_manifesto == '210240' and not justificativa:
                messages.error(self.request, 'Justificativa é obrigatória para manifestação de evento: \'Operação não Realizada\'')
                return self.form_invalid(form)
            ...
            processo = self.efetuar_manifesto(chave, cnpj, ambiente, tipo_manifesto, justificativa, ambiente_nacional, salvar_arquivos)
            return self.form_valid(form, processo)
