...
class Pessoa(models.Model):
    ...
    #Dados padrao
    endereco_padrao = models.ForeignKey('cadastro.Endereco', related_name="end_padrao", on_delete=models.CASCADE, null=True, blank=True)
    telefone_padrao = models.ForeignKey('cadastro.Telefone', related_name="tel_padrao", on_delete=models.CASCADE, null=True, blank=True)
    site_padrao     = models.ForeignKey('cadastro.Site', related_name="sit_padrao", on_delete=models.CASCADE, null=True, blank=True)
    email_padrao    = models.ForeignKey('cadastro.Email', related_name="ema_padrao", on_delete=models.CASCADE, null=True, blank=True)
    banco_padrao    = models.ForeignKey('cadastro.Banco', related_name="ban_padrao", on_delete=models.CASCADE, null=True, blank=True)
...
class PessoaJuridica(models.Model):
    pessoa_id           = models.OneToOneField(Pessoa, on_delete=models.CASCADE, primary_key=True, related_name='pessoa_jur_info')
...
class Endereco(models.Model):
    pessoa_end    = models.ForeignKey(Pessoa, related_name="endereco", on_delete=models.CASCADE)
    ...
    cpais         = models.CharField(max_length=5, null=True, blank=True, default='1058')
    ...
    def __unicode__(self):
        s = u'%s, %s, %s (%s)' %(self.logradouro, self.numero, self.municipio, self.uf)
        return s
    def __str__(self):
        s = u'%s, %s, %s (%s)' %(self.logradouro, self.numero, self.municipio, self.uf)
        return s
...
class Telefone(models.Model):
    pessoa_tel      = models.ForeignKey(Pessoa, related_name="telefone", on_delete=models.CASCADE)
...
class Email(models.Model):
    pessoa_email    = models.ForeignKey(Pessoa, related_name="email", on_delete=models.CASCADE)
    email           = models.CharField(max_length=255)


class Site(models.Model):
    pessoa_site = models.ForeignKey(Pessoa, related_name="site", on_delete=models.CASCADE)
    site        = models.CharField(max_length=255)

class Banco(models.Model):
    pessoa_banco = models.ForeignKey(Pessoa, related_name="banco", on_delete=models.CASCADE)
    ...
    def __unicode__(self):
        s = u'%s / %s / %s' %(self.get_banco_display(), self.agencia, self.conta)
        return s
    def __str__(self):
        s = u'%s / %s / %s' %(self.get_banco_display(), self.agencia, self.conta)
        return s
...
class Documento(models.Model):
    pessoa_documento = models.ForeignKey(Pessoa, related_name="documento", on_delete=models.CASCADE)
