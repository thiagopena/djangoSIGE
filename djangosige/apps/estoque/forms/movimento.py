...
class MovimentoForm(forms.ModelForm):
    ...
    class Meta:
        fields = ('data_movimento', 'quantidade_itens', 'valor_total', 'observacoes',)
        widgets = {
...
class EntradaEstoqueForm(MovimentoForm):
    ...
    class Meta(MovimentoForm.Meta):
        ...
        fields = MovimentoForm.Meta.fields + ('tipo_movimento', 'pedido_compra', 'fornecedor', 'local_dest',)
        widgets = MovimentoForm.Meta.widgets
        widgets['tipo_movimento'] = forms.Select(attrs={'class':'form-control'})
        widgets['pedido_compra'] = forms.Select(attrs={'class':'form-control'})
...
class SaidaEstoqueForm(MovimentoForm):
    ...
    class Meta(MovimentoForm.Meta):
        model = SaidaEstoque
        fields = MovimentoForm.Meta.fields + ('tipo_movimento', 'pedido_venda', 'local_orig',)
        widgets = MovimentoForm.Meta.widgets
        widgets['tipo_movimento'] = forms.Select(attrs={'class':'form-control'})
...
class TransferenciaEstoqueForm(MovimentoForm):
    ...
    class Meta(MovimentoForm.Meta):
        ...
        fields = MovimentoForm.Meta.fields + ('local_estoque_orig', 'local_estoque_dest',)
        widgets = MovimentoForm.Meta.widgets
        widgets['local_estoque_orig'] = forms.Select(attrs={'class':'form-control'})
        widgets['local_estoque_dest'] = forms.Select(attrs={'class':'form-control'})
        labels = MovimentoForm.Meta.labels
...
class ItensMovimentoForm(forms.ModelForm):
    estoque_atual = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control', 'readonly':True}), label='Estoque atual', required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['produto'].choices = ((prod.id,  str(prod)) for prod in Produto.objects.filter(controlar_estoque=True))
...
ItensMovimentoFormSet = inlineformset_factory(MovimentoEstoque, ItensMovimento, form=ItensMovimentoForm, extra=1, can_delete=True)