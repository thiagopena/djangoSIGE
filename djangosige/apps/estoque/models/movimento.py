...
TIPOS_MOVIMENTO_ENTRADA = (
    ("0", "Ajuste"),
    ("1", "Entrada por pedido de compra"),
    ("2", "Entrada por importação de nota fiscal de fornecedor"),
    ("3", "Ajuste inicial"),
)

TIPOS_MOVIMENTO_SAIDA = (
    ("0", "Ajuste"),
    ("1", "Saída por pedido de venda"),
    ("2", "Saída por importação de nota fiscal"),
)
...
class ItensMovimento(models.Model):
    produto = models.ForeignKey(
        "cadastro.Produto",
        related_name="moviment_estoque_produto",
    ...
    def format_estoque_atual_produto(self):
        ...
        if isinstance(estoque_atual, Decimal):
            return locale.format("%.2f", estoque_atual, 1)
        else:
...
class MovimentoEstoque(models.Model):
    ...
    def format_quantidade_itens(self):
        return locale.format("%.2f", self.quantidade_itens, 1)

    def format_valor_total(self):
        return locale.format("%.2f", self.valor_total, 1)


@receiver(pre_delete, sender=MovimentoEstoque, dispatch_uid="movimento_estoque_delete")
def reajusta_estoque_produtos(sender, instance, using, **kwargs):
    ...
    for item in instance.itens_movimento.all():
        ...
        for produto_estocado in produto.produto_estocado.all():
            produto_estocado.quantidade = operation(
                produto_estocado.quantidade, item.quantidade
            )
...
class EntradaEstoque(MovimentoEstoque):
    ...
    pedido_compra = models.ForeignKey(
        ...
        related_name="entrada_estoque_pedido",
    ...
    fornecedor = models.ForeignKey(
        "cadastro.Fornecedor",
        related_name="entrada_estoque_fornecedor",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
    )
    local_dest = models.ForeignKey(
        ...
        related_name="entrada_estoque_local",
    ...
    def get_edit_url(self):
        return reverse_lazy(
            "estoque:detalharentradaestoqueview", kwargs={"pk": self.id}
        )
...
class SaidaEstoque(MovimentoEstoque):
    ...
    local_orig = models.ForeignKey(
        "estoque.LocalEstoque",
        related_name="saida_estoque_local",
        default=DEFAULT_LOCAL_ID,
        on_delete=models.PROTECT,
    )
...
class TransferenciaEstoque(MovimentoEstoque):
    local_estoque_orig = models.ForeignKey(
        "estoque.LocalEstoque",
        related_name="transf_estoque_orig",
        on_delete=models.CASCADE,
    )
    local_estoque_dest = models.ForeignKey(
        "estoque.LocalEstoque",
        related_name="transf_estoque_dest",
        on_delete=models.CASCADE,
    )

    def get_edit_url(self):
        return reverse_lazy(
            "estoque:detalhartransferenciaestoqueview", kwargs={"pk": self.id}
        )
